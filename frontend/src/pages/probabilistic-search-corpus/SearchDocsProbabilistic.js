...
export default function SearchDocsProbabilistic() {
  ...
  const handleSubmit = async () => {
    try {
      if (!!queryString.trim()) {
        const response = await api.post(`/corpus/${uuid}/probabilistic-search`, {
          query: queryString.trim().toLowerCase()
        });
        setDocsList([ ...response.data.docsList ]);
        setLastSearch( [ ...queryString.trim().toLowerCase().split(/\s+/) ] );
        setAlertSearch(true);
      } else {
        setAlertInfo(true);
      }
  ...
  const handleRelevantVote = async (type, docId) => {
    try {
  ...
  const DocsTableMemo = useMemo(
    () => 
      <DocsTable 
        ...
        onRelevantVote={handleRelevantVote}
        score={true} 
        corpus_uuid={uuid}
      />, 
  [docsList, lastSearch]);
  ...
  return (
    <>
      { 
        alertError ? (
          ...
          <Alert severity="error">
            Você precisa gerar o index com IDF antes <b>:\</b>
          </Alert>
        ) : ('') 
      }
      { 
      ...
      <Snackbar open={alertFeedback} autoHideDuration={6000} onClose={() => setAlertFeedback(false)}>
        <Alert onClose={() => setAlertFeedback(false)} severity="success">
          Feedback salvo com sucesso +_+ !!!
        </Alert>
      </Snackbar>
